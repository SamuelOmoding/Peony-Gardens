        ...
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 space-y-8 z-40">
          {links.map(({ id, link, path }) => (
            <li key={id} className="text-2xl text-yellow-600 hover:text-white transition duration-200 ease-in-out">
              <NavLink to={path} onClick={() => setNav(false)}>
                {link}
              </NavLink>
            </li>
          ))}
...
//   return (
//     <div className='flex justify-between items-center w-full h-20 px-4 text-white bg-gray-700 bg-opacity-85 fixed top-1 z-5 shadow-md'>
//   <div>
//     <img src={Logo} alt="logo" className="ml-4 h-12 w-auto" />
//   </div>
